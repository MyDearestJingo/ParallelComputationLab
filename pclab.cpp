...
//优化参数变量
    ...
    */
    ...
    #define N 1000000 // target: 1000000000
    #define SEED 0.3
    /* ================ */
    void sort_gen(int *d, const long N, int seed){
        srand(seed);
        for(long i=0;i<N;i++){
            d[i]=rand();
        }
    }
    void radix_sort(int *arr, const long N, const int radix, const int pos_len, const int buf_size);
#endif SORT
...
int main(){
    ...
    void radix_sort(int *arr, const long N, const int radix, const int pos_len, const int buf_size){
        ...
        for(int i=0;i<n_seg;i++){ // outside loop for different segments' radix sorting
            for(int j=0;j<N;j++){ // inside loop for simple sort of every segment position
                int i_bin = // get the target segment
